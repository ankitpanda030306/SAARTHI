...
import com.vaadin.flow.component.formlayout.FormLayout; // IMPORTED FOR THE ADMISSION FORM
...
public class PrincipalDashboard extends VerticalLayout {
    ...
    public PrincipalDashboard(StudentService studentService, AuthService authService, UserRepository userRepository, SchemeService schemeService) {
        ...
        Tab tab4 = new Tab("Govt Schemes Library"); 
        ...
        tabs.addSelectedChangeListener(event -> {
            ...
            else content.add(createSchemeSection()); 
        });
    ...
    private Component createStudentSection(User user) {
        ...
        // ==========================================
        // UPDATED: NEW ADMISSION BUTTON & DIALOG FIX
        // ==========================================
        Button addBtn = new Button("New Admission", VaadinIcon.PLUS.create());
        addBtn.addThemeVariants(ButtonVariant.LUMO_PRIMARY);

        Dialog admissionDialog = new Dialog();
        admissionDialog.setHeaderTitle("New Student Admission Profile");
        admissionDialog.setWidth("450px");

        FormLayout formLayout = new FormLayout();
        TextField firstName = new TextField("First Name");
        TextField lastName = new TextField("Last Name");
        TextField rollNo = new TextField("Roll Number");
        TextField studentClass = new TextField("Class / Grade");
        ComboBox<String> gender = new ComboBox<>("Gender");
        gender.setItems("Male", "Female", "Other");
        TextField guardianIncome = new TextField("Annual Family Income (₹)");
        ComboBox<String> riskLevel = new ComboBox<>("Initial Risk Assessment");
        riskLevel.setItems("Low", "Medium", "High", "Critical");

        formLayout.add(firstName, lastName, rollNo, studentClass, gender, guardianIncome, riskLevel);
        admissionDialog.add(formLayout);

        Button saveButton = new Button("Save Profile", ev -> {
            if(firstName.isEmpty() || lastName.isEmpty()) {
                Notification.show("Please fill out name fields").addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
            }
            
            Student newStudent = new Student();
            newStudent.setFullName(firstName.getValue() + " " + lastName.getValue());
            newStudent.setRollNumber(rollNo.getValue());
            newStudent.setCurrentClass(studentClass.getValue());
            newStudent.setRiskLevel(riskLevel.getValue() != null ? riskLevel.getValue() : "Low");
            
            // Uncomment the line below if your Student entity has a setSchool method to link it to the Principal's school
            // newStudent.setSchool(user.getSchool()); 

            try {
                // Uncomment this when you have added a save() method in your StudentService
                ...
                Notification.show("Student Profile Saved!").addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                admissionDialog.close();
            } catch (Exception exception) {
                Notification.show("Error saving student. Check database connection.").addThemeVariants(NotificationVariant.LUMO_ERROR);
            }
        });
        saveButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY, ButtonVariant.LUMO_SUCCESS);

        Button cancelButton = new Button("Cancel", ev -> admissionDialog.close());
        cancelButton.addThemeVariants(ButtonVariant.LUMO_TERTIARY);

        admissionDialog.getFooter().add(cancelButton, saveButton);

        addBtn.addClickListener(e -> admissionDialog.open());
        // ==========================================
        // END OF NEW ADMISSION DIALOG LOGIC
        // ==========================================
            ...
            logGrid.addComponentColumn(c -> {
                if (c.isRead()) {
                    ...
                    resolveBtn.addClickListener(e -> {
                        studentService.markComplaintAsRead(c); 
